...
namespace :db do
  ...
  task :pull do
    on primary :db do |host|
      ...
      remote_database_config = YAML.load(database_yml)
      local_database_config = YAML.load(File.read('config/database.yml'))

      remote_database = remote_database_config[fetch(:rails_env).to_s]
      local_database = local_database_config['development']
      ...
      case remote_database['adapter']
        ...
        when 'postgresql'
          ...
          cmd = "bzip2 -cd #{fh.path} | psql -U #{local_database['username']} -d #{local_database['database']}"
          info cmd
          system cmd
          ...
          raise "Adapter #{remote_database_config['adapter']} not supported."
      end
    end
  end
